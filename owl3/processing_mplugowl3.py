...
class mPLUGOwl3Processor(ProcessorMixin):
    ...
    def __init__(self, image_processor: mPLUGOwl3ImageProcessor = None, tokenizer=None, prompt_style='chatml',
                 inference_mode=True, addition_eod="<|endoftext|>"):
    ...
    def encode_text_sft(self, texts):
        ...
        for i, _ in enumerate([media_helper]):
            ...
            media_offset.append(torch.cat(
                [(torch.ones(mo.shape[0], 1) * media_before).long().to(mo.device), (mo + media_before).unsqueeze(1)],
                dim=1))  # L 2
        ...
        if images is not None:
            ...
            if image_inputs.get('cut_shape', None) is not None:
                ...
                for message in messages:
                    ...
                    text = text_list[0]
                    for next_text in text_list[1:]:
                        text += self.image_processor.cut_prompt_template(img_token='<|image|>',
                                                                         h=cut_shape[image_token_ptr][0],
                                                                         w=cut_shape[image_token_ptr][1])
                        text += next_text
                        image_token_ptr += 1
                    message['content'] = text
